...
-record(state, {}).
-record(session, {id, user, pin, name}).
    ...
    {ehtml,
     {form, [{method, get},
      ...
      {table, [{width, "42%"},
               ...
               {cellpadding, "0"}],
       [{tr, [],
         [{td, [{width, "10%"}, {class, "body"}], "Account Number"},
          {td, [{width, "90%"}], {input, [{name, user},
                                          {type, text}]}}]},
        {tr, [],
         [{td, [{width, "10%"}, {class, "body"}], "Pin"},
...
%% @spec () -> 
%
post_process(login, [UserStr, Pin], A) ->
    User = list_to_integer(UserStr),
    case backend:pin_valid(User, Pin) of
        true ->
            #account{name = Name} = backend:account(User),
            Session = #session{user = User, pin = Pin, name=Name},
...
post_process(transfer, [Amount, From, To], A) ->
    ...
    case backend:transfer(From1, To1, Amount1, Session#session.pin) of
...
transactions_accounts(A) ->
    ...
    lists:foldl(fun(#account{acc_no = No, balance = B, transactions = T}, Acc) ->
                        [{table, [{width, "250"},
                                  {border, "0"},
                                  {cellspacing, "0"},
                                  {cellpadding, "2"}],
...
%% @spec () -> 
%
transactions_trans1(Type, {{Year, Month, Day}, {Hour, Minute, Seconds}}, Sum) ->
    ...
    {tr, [], [{td, [{class, "body"}, {width, "94"}], Date},
              case Type of
                  credit -> {td, [{class, "body"}, {width, "14"}], "&nbsp"};
                  debit -> {td, [{class, "body"}, {width, "14"}], "-"}
              end,
...
accounts_get_accounts(A) ->
    {ok, Session, _Cookie} = check_cookie(A),
    Accounts = backend:accounts_by_name(Session#session.name),
    TRA =
    fun(No, Amount) ->
        {tr, [],
    ...
    lists:foldl(fun(#account{balance = B, acc_no = No}, {Total, List}) ->
